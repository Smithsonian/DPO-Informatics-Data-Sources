#!/usr/bin/env python3
#
# Get geoBoundaries data via the API
#
# v. 2021-11-24
#
...
#Download main json index from:
api_url = "https://www.geoboundaries.org/api/current/gbOpen/ALL/ALL/"

#response = urlopen(api_url, context=ssl.create_default_context(cafile=certifi.where()))
# Download using wget or something else. There seems to be an issue with the cert
...
if __name__ == '__main__':
    ...
    cur.execute("DELETE FROM wikidata_records WHERE latitude = round(latitude::numeric);")
    cur.execute("DELETE FROM wikidata_records WHERE latitude = round(latitude::numeric, 1);")
    cur.execute("DELETE FROM wikidata_records WHERE latitude = round(latitude::numeric, 2);")
    cur.execute("DELETE FROM wikidata_records WHERE longitude = round(longitude::numeric);")
    cur.execute("DELETE FROM wikidata_records WHERE longitude = round(longitude::numeric, 1);")
    cur.execute("DELETE FROM wikidata_records WHERE longitude = round(longitude::numeric, 2);")

    cur.execute("CREATE INDEX wikidata_names_id_idx ON wikidata_names USING btree (source_id);")
    cur.execute("DELETE FROM wikidata_names WHERE source_id NOT IN (SELECT source_id FROM wikidata_records);")
    ...
    cur.execute("DELETE FROM wikidata_descrip WHERE source_id NOT IN (SELECT source_id FROM wikidata_records);")
    ...
    #cur.execute("UPDATE data_sources SET no_features = w.no_feats FROM (select count(*) as no_feats from
    # wikidata_names) w WHERE datasource_id = 'wikidata'")
    cur.close()
    conn.close()
    end_time = time.time()
