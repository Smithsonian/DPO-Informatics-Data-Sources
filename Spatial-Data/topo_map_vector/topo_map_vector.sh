...
#Rename cols
psql -c "ALTER TABLE topo_map_vector_county RENAME column source_d_1 TO source_des;"
psql -c "ALTER TABLE topo_map_vector_derivednames ADD column globalid text;"
psql -c "ALTER TABLE topo_map_vector_firstdiv RENAME column source_d_1 TO source_des;"
psql -c "ALTER TABLE topo_map_vector_firstdiv ADD column globalid text;"
psql -c "ALTER TABLE topo_map_vector_elev RENAME column source_d_1 TO source_des;"
psql -c "ALTER TABLE topo_map_vector_waterbody DROP column the_geom;"
psql -c "ALTER TABLE topo_map_vector_waterbody ADD column the_geom geometry(MultiPolygon, 4326);"
...
psql -c "UPDATE data_sources SET is_online = 'f' WHERE datasource_id = 'topo_map_vector';"
...
cd county
psql -c "TRUNCATE topo_map_vector_county"
for i in *.sql; do
...
cd derived
psql -c "TRUNCATE topo_map_vector_derivednames"
for i in *.sql; do
    #Load PostGIS files to database
    echo $i
    #Rename col
...
cd firstdiv
psql -c "TRUNCATE topo_map_vector_firstdiv"
for i in *.sql; do
    #Load PostGIS files to database
    echo $i
    #Rename col
...
# cd nhdpoint
# psql -c "TRUNCATE topo_map_vector_nhdpoint"
# for i in *.sql; do
#     #Load PostGIS files to database
#     echo $i
#     #Rename col
...
cd waterbody
psql -c "TRUNCATE topo_map_vector_waterbody"
for i in *.sql; do
    #Load PostGIS files to database
    echo $i
    #Rename col
    ...
    psql -v ON_ERROR_STOP=1 -c "DELETE FROM topo_map_vector_waterbody WHERE gnis_name IS NULL"
    psql -v ON_ERROR_STOP=1 -c "VACUUM topo_map_vector_waterbody"
done
cd ..


cd elev
psql -c "TRUNCATE topo_map_vector_elev"
for i in *.sql; do
    #Load PostGIS files to database
    echo $i
    #Rename col
...
#
psql -v ON_ERROR_STOP=1 -c "CREATE INDEX topo_map_vector_elev_elev_idx ON topo_map_vector_elev USING BTREE(contourele);"
psql -v ON_ERROR_STOP=1 -c "DELETE FROM topo_map_vector_elev WHERE RIGHT(contourele::int::text, 2) != '50' AND RIGHT(contourele::int::text, 2) != '00'"
psql -v ON_ERROR_STOP=1 -c "VACUUM topo_map_vector_elev"
#
cd ..
