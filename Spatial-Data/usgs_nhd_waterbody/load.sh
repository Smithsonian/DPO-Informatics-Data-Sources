...
psql -c "UPDATE data_sources SET is_online = 'f' WHERE datasource_id = 'usgs_nhd_waterbody';"
...
psql -c "DELETE FROM usgs_nhd_waterbody;"
psql -c "VACUUM usgs_nhd_waterbody;"
...
#psql -c "WITH data AS (SELECT gid, ST_Force2D(ST_TRANSFORM(geom, 4269, 4326)) as geom from usgs_nhd_waterbody) UPDATE usgs_nhd_waterbody u SET the_geom = d.geom FROM data d where u.gid = d.gid;"
#-s 4269:4326
#psql -c "UPDATE nhd_waterbody SET the_geom = ST_Force2D(geom);"


#indices and new columns
psql < post.sql

psql -c "UPDATE data_sources SET is_online = 't', source_date = '$script_date', no_features = w.no_feats FROM (select count(*) as no_feats from usgs_nhd_waterbody) w WHERE datasource_id = 'usgs_nhd_waterbody';"
