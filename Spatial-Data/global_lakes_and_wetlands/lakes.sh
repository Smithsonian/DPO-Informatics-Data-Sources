...
psql -c "UPDATE data_sources SET is_online = 'f' WHERE datasource_id = 'global_lakes';"
...
psql -c "DROP TABLE global_lakes CASCADE;"
...
psql -c "UPDATE data_sources SET is_online = 't', source_date = '$script_date', no_features = w.no_feats FROM (select count(*) as no_feats from global_lakes) w WHERE datasource_id = 'global_lakes';"
