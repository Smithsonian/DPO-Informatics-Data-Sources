...
ALTER TABLE wdpa_polygons ADD COLUMN gadm_country text;
ALTER TABLE wdpa_polygons ADD COLUMN gadm2 text;
ALTER TABLE wdpa_points ADD COLUMN gadm_country text;
ALTER TABLE wdpa_points ADD COLUMN gadm2 text;
...
WITH data AS (
    ...
    FROM 
        ...
        gadm2 g
    WHERE 
        ST_INTERSECTS(w.the_geom, g.the_geom_simp)
    GROUP BY 
        w.uid
)
UPDATE wdpa_polygons g SET gadm2 = d.loc, gadm_country = d.name_0 FROM data d WHERE g.uid = d.uid;
...
UPDATE wdpa_points g SET gadm2 = d.loc, gadm_country = d.name_0 FROM data d WHERE g.uid = d.uid;
...
CREATE INDEX wdpa_polygons_gadmc_idx ON wdpa_polygons USING gin (gadm_country gin_trgm_ops);
CREATE INDEX wdpa_points_gadmc_idx ON wdpa_points USING gin (gadm_country gin_trgm_ops);
