...
psql -c "UPDATE data_sources SET is_online = 'f' WHERE datasource_id = 'usa_histplaces_points';"
psql -c "UPDATE data_sources SET is_online = 'f' WHERE datasource_id = 'usa_histplaces_poly';"
...
# psql -c "DROP TABLE usa_histplaces CASCADE;"
...
psql -c "UPDATE data_sources SET is_online = 't', source_date = '$script_date', no_features = w.no_feats FROM (select count(*) as no_feats from usa_histplaces_points) w WHERE datasource_id = 'usa_histplaces_points';"

psql -c "UPDATE data_sources SET is_online = 't', source_date = '$script_date', no_features = w.no_feats FROM (select count(*) as no_feats from usa_histplaces_poly) w WHERE datasource_id = 'usa_histplaces_poly';"
